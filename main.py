...
while True:
    ...
    if desc:
        print("It is%s dark.\nYou can %s\nThere are these objects: %s\nYou can see %s\nThere are these people/monsters: %s\nWhat do you do?" % (\
            ...
            'not exit' if len(croom['exits']) == 0 else 'exit %s' % ", ".join(["%s towards %s" % (pos[int(i)], \
            fc["rooms"][str(croom['exits'][i])]["name"]) for i in croom['exits']]), \
