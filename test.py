...
class MainGameTest(unittest.TestCase):
    ...
    def test_run_action(self):
        s.run_action("C*G*['apple'] = '{1}'", ['a', 'b', 'c'], set_values_3=False)
        self.assertEqual(s.get_globals()['apple'], 'b')

        s.run_action("C*G*['apple'] = {3}", ['a', 'b', [0, 1, 2, 3], None])
        self.assertEqual(s.get_globals()['apple'], 0)
        
        s.run_action("C*G*['apple'] = int('{3}')", ['a', 'b', [5, 10], 'c'])
        self.assertEqual(s.get_globals()['apple'], 5)
        
        self.assertRaises(IndexError, s.run_action, "C*G*['apple'] = '{5}'", ['a', 'b', [0, 1, 2, 3], None])
        
        #self.assertRaises(NameError, s.run_action, "C*G*['apple'] = '{5}'", ['a', 'b', [0, 1, 2, 3], None])
